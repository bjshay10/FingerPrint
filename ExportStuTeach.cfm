<!--- Export Student Teacher data for Finger Print DB 
		Create On 12/03/14
		By: B.J. Shay 
		Modification:
		12/03/14 - BJS - Initial Creation --->
...
<cfquery name="GetFPData" datasource="hrinfopath">
	SELECT     FPID, LName, FName, SSN, FormerLName, DOB, FPDate, FPUnRead, Attachment_yn, Undetermined_yn, Misdemeanor_yn, Felony_yn, Comment, DoNotRehire, Address, 
                      City, State, Zip, Phone, EMail, Volunteer, UpdatedBy, UpdatedOn, StudentTeacher, StudentTeacher, STA, StudentTeacherCDE
	FROM       tblFingerPrintDB
    WHERE		StudentTeacher = 'on' or StudentTeacherCDE = 'on'
</cfquery>
